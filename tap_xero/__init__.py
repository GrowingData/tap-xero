...
CREDENTIALS_KEYS = ["consumer_key", "consumer_secret", "rsa_key"]
...
def discover(config):
    ...
    for stream in streams_.all_streams:
        ...
        catalog.streams.append(
            CatalogEntry(
                stream=stream.tap_stream_id, tap_stream_id=stream.tap_stream_id, key_properties=stream.pk_fields, schema=schema, metadata=mdata
            )
        )
    return catalog
...
def load_and_write_schema(stream):
    singer.write_schema(stream.tap_stream_id, load_schema(stream.tap_stream_id), stream.pk_fields)
...
def sync(ctx):
    ...
    start_idx = streams_.all_stream_ids.index(currently_syncing) if currently_syncing else 0
    stream_ids_to_sync = [cs.tap_stream_id for cs in ctx.catalog.streams if cs.is_selected()]

    streams = [s for s in streams_.all_streams[start_idx:] if s.tap_stream_id in stream_ids_to_sync]
...
def main_impl():
    ...
    if args.discover:
        ...
        # catalog = Catalog.from_dict(args.properties) if args.properties else discover(args.config)
